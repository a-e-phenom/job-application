...
const VoiceScreeningStep = React.memo(function VoiceScreeningStep({
  ...
  const renderIntroStep = () => {
    ...
    // Split screen layout with image - force vertical on mobile
    ...
    return (
      <div className="w-full bg-white flex items-center justify-center m-0 p-0" style={{ minHeight: 'calc(100vh - 140px)' }}>
        <div className={`grid ${isMobileView ? 'grid-cols-1 gap-6' : 'grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16'} items-start w-full max-w-6xl mx-auto ${isMobileView ? 'px-4' : 'px-6 md:px-8 lg:px-12'}`}>
          {imagePosition === 'left' ? (
            <>
              {/* Image on the left */}
              <div className="order-2 lg:order-1">
                <img
                  src={introImage}
                  alt="Voice screening"
                  className={`w-full h-auto object-contain rounded-lg ${isMobileView ? 'max-h-[300px]' : 'max-h-[500px]'}`}
                />
              </div>
              {/* Content on the right */}
              <div className="order-1 lg:order-2">
                <h2 className={`${isMobileView ? 'text-[20px]' : 'text-[28px]'} font-semibold text-[#353B46] mb-4`}>
                  {defaultTitle}
                </h2>

                <div className={`space-y-3 ${isMobileView ? 'text-[14px]' : 'text-[16px]'} text-[#464F5E] leading-relaxed`}>
                  {/* How to prepare section */}
                  <div>
                    <h3 className="font-semibold text-[#353B46] mb-1">How to prepare:</h3>
                    <p>
                  ...
                  {/* Support message */}
                  <div className={`bg-gray-50 rounded-xl px-4 py-2 mt-6 ${isMobileView ? 'mt-3' : 'mt-6'}`}>
                    <p className={`${isMobileView ? 'text-[12px]' : 'text-[14px]'} text-[#464F5E]`}>
                      If you have any technical questions or need support with your agent call, don't hesitate to{' '}
                      <a href="#" className="text-blue-600 underline hover:text-blue-700">
                        contact our candidate support team
                      </a>
            ...
            <>
              ...
              <div className="order-1 lg:order-1">
                ...
                <div className={`space-y-6 text-[#464F5E] leading-relaxed ${isMobileView ? 'text-[14px]' : 'text-[16px]'}`}>
                  {/* How to prepare section */}
                  <div>
                    <h3 className={`font-semibold text-[#353B46] ${isMobileView ? 'mb-1' : 'mb-2'}`}>How to prepare:</h3>
                    <p>
                      {template?.content?.subtitle || "The screening takes around 10 minutes. We recommend completing the screening in a quiet space where you can focus and be easily heard."}
                    </p>
                  </div>
                  ...
                  {/* During the call section */}
                  <div>
                    <h3 className={`font-semibold text-[#353B46] ${isMobileView ? 'mb-1' : 'mb-2'}`}>During the call</h3>
                    <p>
                      You'll have a natural, voice-based conversation with our virtual agent. Feel free to ask questions or request more info about the process at any time.
                    </p>
                  </div>
                  ...
                  {/* After the call section */}
                  <div>
                    <h3 className={`font-semibold text-[#353B46] ${isMobileView ? 'mb-1' : 'mb-2'}`}>After the call</h3>
                    <p>
                      Once you finish, our team will review your responses and get back to you shortly with the next steps.
                    </p>
                  </div>
                  ...
                  {/* Support message */}
                  <div className={`bg-gray-50 border border-gray-200 rounded-lg p-4 mt-6 ${isMobileView ? 'mt-3 p-3' : ''}`}>
                    <p className={`${isMobileView ? 'text-[12px]' : 'text-[14px]'} text-[#464F5E]`}>
                      If you have any technical questions or need support with your agent call, don't hesitate to{' '}
                      <a href="#" className="text-blue-600 underline hover:text-blue-700">
                        contact our candidate support team
  ...
  const renderCallStep = () => {
    return (
      <div className="w-full m-0 p-0 overflow-x-hidden flex flex-col" style={{ minHeight: 'calc(100vh - 140px)', backgroundColor: '#F8F9FB' }}>
        <div className={`flex-1 flex ${isMobileView ? 'items-start' : 'items-center'} justify-center`}>
          <div className={`max-w-4xl w-full ${isMobileView ? 'px-4 py-20' : 'px-8'}`}>
            <div className={`grid ${isMobileView ? 'grid-cols-1 gap-6' : 'grid-cols-1 md:grid-cols-2'} mb-8`} style={{ gap: isMobileView ? '24px' : '48px' }}>
              {/* Virtual Agent Card */}
              <div
                ...
                style={{
                  ...
                  minHeight: isMobileView ? '160px' : '200px',
                  paddingTop: '12px',
                  paddingBottom: '12px',
                  paddingLeft: '12px',
                  paddingRight: '12px'
              ...
              >
              <h3 className={`${isMobileView ? 'text-[14px]' : 'text-[16px]'} font-semibold text-[#353B46] mb-2`}>
                Rachel
              </h3>
              <p className={`${isMobileView ? 'text-[12px]' : 'text-[14px]'} font-normal text-[#637085] mb-4`}>
                Virtual agent
              </p>
                ...
                {!isRachelInactive && (
                  <div className="flex items-center justify-center">
                    <div className="flex items-end justify-center space-x-0.5" style={{ height: isMobileView ? '24px' : '32px' }}>
                      {Array.from({ length: isMobileView ? 20 : 28 }, (_, i) => {
            ...
            >
              ...
              style={{ color: isDavidActive ? primaryColor : '#353B46', fontSize: isMobileView ? '14px' : '16px' }}>
                David
              </h3>
              <p className={`${isMobileView ? 'text-[12px]' : 'text-[14px]'} font-normal text-[#637085] mb-4`}>
                You
              ...
              {isDavidActive && (
                <div className="flex items-center justify-center">
                  <div className="flex items-end justify-center space-x-0.5" style={{ height: isMobileView ? '24px' : '32px' }}>
                    {Array.from({ length: isMobileView ? 15 : 20 }, (_, i) => {
                      const waveClasses = ['animate-audio-wave-1', 'animate-audio-wave-2', 'animate-audio-wave-3', 'animate-audio-wave-4', 'animate-audio-wave-5'];
                      const waveClass = waveClasses[i % waveClasses.length];
        ...
        <div className="text-center pb-4">
          <p className={`${isMobileView ? 'text-[14px]' : 'text-[16px]'} font-normal text-[#637085] mb-4`}>
  ...
  const memoizedMobileFooter = React.useMemo(() => {
    if (!isMobileView) return null;

    return (
      <div className="w-full pb-4 bg-white border-t border-gray-200 pt-3">
        <div className="px-4">
          <button
            onClick={() => {
              if (currentSubStep === 0) {
            ...
            className="w-full h-[44px] flex items-center justify-center space-x-2 px-4 rounded-[10px] transition-all duration-200 text-white text-sm"
  ...
  }, [isMobileView, currentSubStep, primaryColor, handleChange, setCurrentSubStep, localData, onUpdate, onNext]);

  // Call footer render callback if mobile
  useEffect(() => {
